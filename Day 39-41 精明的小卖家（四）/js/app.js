...
function refreshOutput() { // 筛选并显示结果
    ...
    setHashFromFilters(); // 因为 filter 的每次变动都会调用 refreshOutput()，所以 setHashFromFilters() 在这里被调用。同时，"输出更新时重设 hash"也很符合逻辑
...
window.onload = function () {
    ...
    default_line_graph_color = line_setting.graph_color; // 从其他 js 中读取绘图颜色初始值
    default_bar_graph_color = bar_setting.graph_color;

    refreshOutput(); // 初始显示一次数据
}

window.onhashchange = function () { // hash.js 会用到的函数
    filterByHash();
    refreshOutput();
}

/*
window.onpopstate = function () { // pushstate.js 会用到的函数
    filterByHash();
    refreshOutput();
}
*/
