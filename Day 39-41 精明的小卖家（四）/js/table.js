...
function isInTheFilter(value, filter_data) {
    var result = false;
    for (let i = 0; i < filter_data.length; i++) {
        if (value == filter_data[i]) {
            result = true;
            break;
        }
    }
...
function getDataByFilter(source_data, property, filter_data) {
    var result = [];
...
function createTable(table, data) {
    // 通过 data 计算商品、地区总数，而非依靠输入参数，以使结果准确体现数据
    var region_filter = [],
        product_filter = [];
    ...
    if (num_of_region == 1 && num_of_product > 1) {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            // 为了使得 rowspan 时仍能读取 region 数据，这里没有将其删去，而是设为了不显示。下同
