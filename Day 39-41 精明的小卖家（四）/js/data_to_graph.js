...
function getGraphData(product, region) {
    var data = [];
...
function getDataFromTD(td) {
    ...
    for (; i < all_td.length; i++) {
        data.push(all_td[i].innerText); // 若使用 .innerHTML，在 td 里添加子元素后，也会被写入 data 中，导致运行出错
    }

    /* 以下是"读取所选行的商品、地区，在数据库中查找，使用找到的数据生成图像"的写法
     * 优点：之后可以方便的调取商品、地区名这些数据；数据直接从服务器取得，避开了本地修改
     * 缺点：为了在 rowspan 情况下能顺利读取被合并的数据，要在 table.js 的 createTable() 函数中做特殊处理，详见其注释。如果不这么处理，读取 rowspan 会极其麻烦。
     * --------
    if (document.querySelector("#output_table thead tr").children[0].innerHTML == "商品") {
        ...
        data = getGraphData(product, region);
...
function editMultipleByData(td) { // 根据 td 值增删 multiple 的项
    var data = getDataFromTD(td);
